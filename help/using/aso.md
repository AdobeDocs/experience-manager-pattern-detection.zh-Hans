...
* 数量较多的虚名URL（超过1000）会延长查询时间，从而增加Dispatcher和发布服务器的负载。
* 自定义应用程序可能依赖于 AEM as a Cloud Service 中不可用的产品或特性。
* 升级不支持的功能可能会导致升级失败以及应用程序无法使用。
* 大量处于正在运行或陈旧状态的创作工作流可能会降低性能。
* 查询速度慢可能会降低系统的性能。
...
* 必须处理未处理的资源，并且 `dam:assetState` 上的属性 `jcr:content` 必须将资源的节点设置为“已处理”。 或者，您应该在迁移到 AEMaaCS 之前从迁移集中删除这些资产。
