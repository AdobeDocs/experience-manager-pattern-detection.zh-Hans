...
* `legacy.dialog.classic`：基于 ExtJS 的经典 UI 对话框必须改为 Coral。
   * 当对话框的名称为`dialog`或`design_dialog`并且在
`jcr:primaryType`属性值或`xtype`属性值是`cq:Dialog`。
...
并且`sling:resourceType`属性值不包含`granite/ui/components/coral/foundation`。
* `legacy.custom.component`：继承自 `foundation/components` 的组件应该更新以使用核心组件。
   * 当 `jcr:primaryType` 属性值为 `cq:Component` 和
     ...
     超类型链组件包含的`sling:resourceSuperType`属性值
“基础/组件。”
...
* 在AEM 6.3中，内容片段模板取代了内容片段模型。 将基于旧模板的内容片段迁移到AEM as a Cloud Service后，这些片段仍然可用，但无法基于旧模板创建片段。 也无法使用 AEM GraphQL 提供这些片段，它需要内容片段模型作为架构。
